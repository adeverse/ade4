...
# Function add.graph
#####################
add.graph <- function(func,posi=c("bottomleft","bottomright","topleft","topright"),ratio=.2,inset=.01,bg.col='white'){
...
###########################
"add.scatter.eig" <- function (w, xax, yax, posi = "bottomleft", ratio = .25, inset = .01, sub="Eigenvalues",csub=2*ratio){
  opar <- par("mar","xaxt","yaxt")
  on.exit(par(opar))
  par(mar=rep(.1,4),xaxt="n",yaxt="n")

  fgraph <- function(){
    scatterutil.eigen(w,wsel=c(xax,yax),sub=sub,csub=csub,box=TRUE)
  }
  
  add.graph( fgraph(), posi=posi , ratio=ratio , inset=inset)
