plot.4thcorner <- function(x, stat = c("D", "D2", "G"), type = c("table", "biplot"), xax = 1, yax = 2, x.rlq = NULL, alpha = 0.05, ...) {
  ...
  table4thcorner <- function (df, stat, assignR,assignQ) 
    {
  ...
  biplot.rlq4thcorner <- function (res.4thcorner, obj.rlq, stat, alpha, xax, yax, clab.traits, clab.env) 
    {
      ...
      s.label(coolig, clabel = 0, cpoint = 0)
      ...
      if(length(idx.tot[[1]]) > 0)
        {
          scatterutil.eti(coolig[-idx.tot[[1]],1], coolig[-idx.tot[[1]],2],label=row.names(coolig)[-idx.tot[[1]]], clabel = clab.env, boxes = FALSE, coul = rep('lightgrey', nrow(coolig) - length(idx.tot[[1]])))
          scatterutil.eti(coocol[-idx.tot[[2]],1], coocol[-idx.tot[[2]],2],label=row.names(coocol)[-idx.tot[[2]]], clabel = clab.traits, boxes = FALSE, coul = rep('lightgrey', nrow(coocol) - length(idx.tot[[2]])))
        } else {
          scatterutil.eti(coolig[,1], coolig[,2],label=row.names(coolig), clabel = clab.env, boxes = FALSE, coul = rep('lightgrey', nrow(coolig)))
          scatterutil.eti(coocol[,1], coocol[,2],label=row.names(coocol), clabel = clab.traits, boxes = FALSE, coul = rep('lightgrey', nrow(coocol)))
        }
      
      if(nrow(idx.pos) > 0)
        segments(coolig[idx.pos[,1],1],coolig[idx.pos[,1],2],coocol[idx.pos[,2],1],coocol[idx.pos[,2],2], lty = 2, lwd = 3, col='black')
      
      if(nrow(idx.neg) > 0)
        segments(coolig[idx.neg[,1],1],coolig[idx.neg[,1],2],coocol[idx.neg[,2],1],coocol[idx.neg[,2],2], lty = 3, col = 'black')
  ...
  biplot.axesrlq4thcorner <- function(res.4thcorner, coo, alpha, xax, yax, type.axes){
    opar <- par(mar = par("mar"))
    on.exit(par(opar))
    ...
    if(length(idx.tot) > 0)
      {
        scatterutil.eti(coo[-idx.tot,1], coo[-idx.tot,2],label=row.names(coo)[-idx.tot], clabel = 1, boxes = FALSE, coul = rep('lightgrey', nrow(coo) - length(idx.tot)))
      } else {
        scatterutil.eti(coo[,1], coo[,2],label=row.names(coo), clabel = 1, boxes = FALSE, coul = rep('lightgrey', nrow(coo)))
      }
    
    if(length(idx.pos.xax) > 0)
      segments(coo[idx.pos.xax,1],coo[idx.pos.xax,2],coo[idx.pos.xax,1],0, lty = 2, lwd = 3, col='black')
    
    if(length(idx.pos.yax) > 0)
      segments(coo[idx.pos.yax,1],coo[idx.pos.yax,2],0, coo[idx.pos.yax,2], lty = 2, lwd = 3, col='black')                   
    
    if(length(idx.neg.xax) > 0)
      segments(coo[idx.neg.xax,1],coo[idx.neg.xax,2],coo[idx.neg.xax,1],0, lty = 3, col = 'black')
    
    if(length(idx.neg.yax) > 0)
      segments(coo[idx.neg.yax,1],coo[idx.neg.yax,2],0, coo[idx.neg.yax,2], lty = 3, col = 'black')
    
    if(length(idx.tot) > 0)
      {
        s.label(coo[idx.tot,], add.plot = TRUE)
      }
  }
  
  if(type=="table"){
    table4thcorner(res, stat = stat, assignR = x$assignR, assignQ = x$assignQ)
  } else if(type=="biplot"){
    if(fctn =="fourthcorner" | fctn =="fourthcorner2"){ 
      if (!inherits(x.rlq, "rlq")) 
        stop("'x.rlq' should be of class 'rlq'")
      biplot.rlq4thcorner(res.4thcorner = res, obj.rlq = x.rlq, stat = stat, alpha = alpha, xax = xax, yax = yax, clab.traits = 1, clab.env = 1) 
      ...
      biplot.axesrlq4thcorner(res.4thcorner = res, coo = coo, alpha = alpha, xax = xax, yax = yax, type.axes = type.axes)
    }
  }
}
