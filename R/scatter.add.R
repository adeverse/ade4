...
#####################
# Function scatter.add
#####################
scatter.add <- function(func,posi=c("bottomleft","bottomright","topleft","topright"),ratio=.2,inset=.01,bg.col='white'){
...
"add.scatter.eig" <- function (w, nf=NULL, xax, yax, posi = "bottomleft", ratio = .25, inset = .01, sub="Eigenvalues",csub=2*ratio){
  ...
  scatter.add( fgraph(), posi=posi, ratio=ratio, inset=inset)
