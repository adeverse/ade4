"wgcoia" <- function (dudiX, dudiY, fac, scannf = TRUE) {
####
#### wgCOIA
#### within group coinertia analysis (Franquet et al. 94, 95)
#### Jean Thioulouse, 06 Nov 2009
#### This function takes 2 dudi and one factor. It does a within analysis on each
#### dudi, and does a coinertia analysis on the two within analyses.
####
...
#### between X
	cat("wihtin analysis of dudiX - ")
	witX <- within(dudiX, fac, scannf = scannf)
#### between Y
	cat("within analysis of dudiY - ")
	witY <- within(dudiY, fac, scannf = scannf)
#### coinbet
	cat("within class coinertia analysis\n")
	res <- coinertia(witX, witY, scannf = scannf)
